"""
Dragon Combat Auto-Tuner
Sweeps Vhagar's stats to find the closest 50/50 split vs Caraxes.
Keeps both dragons at 32 total stat points.
Formulas: HP = (Might + Resilience*2) * 10, Strike = Might + Ferocity, Evasion = Agility * 2
"""
import random

def create_dragon(name, might, agility, ferocity, resilience):
    hp = (might + resilience * 2) * 10
    return {
        'name': name, 'might': might, 'agility': agility,
        'ferocity': ferocity, 'resilience': resilience,
        'hp': hp, 'max_hp': hp,
        'strike': might + ferocity, 'evasion': agility * 2,
    }
...
def simulate_fast(dragon_a, dragon_b):
    ...
    while a['hp'] > 0 and b['hp'] > 0:
        ...
        if would_flee(second):
            ...
            if fs > cs: fled = second['name']; break
    ...
    return a['hp'], b['hp'], fled

# Caraxes is FIXED
CARAXES = create_dragon('Caraxes', might=6, agility=10, ferocity=10, resilience=6)
NUM_SIMS = 500

print("Sweeping Vhagar stats (32 point budget, Might 8-10)")
print(f"Formulas: HP=(M+R*2)*10, Strike=M+F, Evasion=A*2")
print(f"Caraxes: M6/A10/F10/R6 -- Strike 16, Evasion 20, HP 180")
print(f"Running {NUM_SIMS} sims per config...")
print()
print(f"{'Config':<22} {'Strike':>6} {'Eva':>4} {'HP':>4} {'DT?':>4} {'C_Win':>6} {'V_Win':>6} {'Mutual':>7} {'Delta':>6}")
print("-" * 75)

results = []

for m in range(8, 11):        # Vhagar Might 8-10
    for a in range(2, 9):      # Vhagar Agility 2-8
        for f in range(5, 11): # Vhagar Ferocity 5-10
            r = 32 - m - a - f # Remaining goes to Resilience
            if r < 1 or r > 10: continue
            if a >= 10 or f > 10: continue

            vhagar = create_dragon('Vhagar', might=m, agility=a, ferocity=f, resilience=r)
            ...
            results.append((delta, m, a, f, r, vhagar['strike'], vhagar['evasion'], vhagar['max_hp'], dt, c_pct, v_pct, m_pct))

# Sort by closest to 50/50
results.sort(key=lambda x: x[0])

for delta, m, a, f, r, strike, eva, hp, dt, c_pct, v_pct, m_pct in results[:25]:
    config = f"M{m}/A{a}/F{f}/R{r}"
    print(f"{config:<22} {strike:>6} {eva:>4} {hp:>4} {dt:>4} {c_pct:>5.1f}% {v_pct:>5.1f}% {m_pct:>6.1f}% {delta:>5.1f}%")

print()
print("Top 5 closest to 50/50:")
for i, (delta, m, a, f, r, strike, eva, hp, dt, c_pct, v_pct, m_pct) in enumerate(results[:5]):
    print(f"  {i+1}. M{m}/A{a}/F{f}/R{r} -- C:{c_pct:.0f}% V:{v_pct:.0f}% Mutual:{m_pct:.0f}% (delta {delta:.1f}%)")
